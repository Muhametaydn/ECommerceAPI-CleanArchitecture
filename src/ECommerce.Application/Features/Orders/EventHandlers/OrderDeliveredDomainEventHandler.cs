...
namespace ECommerce.Application.Features.Orders.EventHandlers
{
    public sealed class OrderDeliveredDomainEventHandler
        : INotificationHandler<DomainEventNotification<OrderDeliveredDomainEvent>>
    {
        ...
        public async Task Handle(
            DomainEventNotification<OrderDeliveredDomainEvent> notification,
            CancellationToken cancellationToken)
        {
            var domainEvent = notification.DomainEvent;

            _logger.LogInformation(
                "Sipariş teslim edildi | OrderId: {OrderId} | OrderNumber: {OrderNumber}",
