...
///
/// ÖNEMLI: GetStringAsync / SetStringAsync, IDistributedCache üzerinde extension metodlardır.
/// Moq extension metodları mock'layamaz. Altta yatan gerçek interface metodları mock'lanmalı:
///   GetAsync(key, ct)  → Task&lt;byte[]?&gt;
///   SetAsync(key, bytes, options, ct)  → Task
...
public class RedisCacheServiceTests
{
    ...
    public RedisCacheServiceTests()
    {
        ...
        _sut = new RedisCacheService(_cacheMock.Object, _multiplexerMock.Object, loggerMock.Object);
    }

    // ── Yardımcı ─────────────────────────────────────────────────────────

    /// <summary>GetStringAsync extension'ı GetAsync(byte[]) çağırır — bunu mock'luyoruz.</summary>
    private void SetupGetAsync(string key, string? jsonValue)
    {
        byte[]? bytes = jsonValue == null ? null : Encoding.UTF8.GetBytes(jsonValue);
        _cacheMock
            .Setup(c => c.GetAsync(key, It.IsAny<CancellationToken>()))
            .ReturnsAsync(bytes);
    }

    /// <summary>SetStringAsync extension'ı SetAsync(byte[]) çağırır — bu metodun çağrıldığını doğruluyoruz.</summary>
    private string? CaptureSetAsyncValue()
    {
        ...
        _cacheMock
            ...
            .Callback<string, byte[], DistributedCacheEntryOptions, CancellationToken>(
                (_, bytes, _, _) => captured = Encoding.UTF8.GetString(bytes))
            .Returns(Task.CompletedTask);
        return null; // captured değer callback ile doldurulur
    }
    ...
    public async Task GetAsync_WhenRedisThrows_ShouldReturnDefaultWithoutThrowing()
    {
        ...
        // Act + Assert — exception dışarı sızmamalı
    ...
    public async Task SetAsync_ShouldSerializeAndStoreValue()
    {
        ...
        var deserialized = JsonSerializer.Deserialize<TestData>(capturedJson!, JsonOpts);
